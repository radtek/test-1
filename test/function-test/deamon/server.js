 ...
 *这种方式也可以,直接传参
 */
var con1 = ps.connect("localhost","admin","admin888");
if(con.hasOwnProperty("errString"))
{
	console.log("错误码:",con1.code);
	console.log("服务器连接失败:",con1.errString);
	throw "connect fail!";
}
/*
 *异步方式
 ...
 *异步直接传参
 */
var res1 = ps.connect("localhost","admin","admin888",function(err,con){
	if(err){
		console.log(err);
	}else{
		console.log("异步直接传参连接成功!");
	}
});
/*
 *判断是否连接：isConnected()
