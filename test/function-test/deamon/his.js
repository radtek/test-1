...
sleep(2000);
//异步插入某一测点历史数据
var resInsertAsy = con.write("/tag/tag4.his.insert",histry1,function(err,tagName,tagId){
	if(err){
		console.log(err);
	}else{
...
//异步直接传参插入某一点历史数据
var resInsertAsy1 = con.write("/tag/tag4.his.insert",12,0,time,function(err,tagName,tagId){
	if(err){
		console.log(err);
	}else{
		console.log("异步插入测点:",tagName,"历史数据成功.");
...
//同步插入替换
var resInsertRep = con.write("/tag/tag3/tag5.his.insertReplace",histry2);
...
//同步直接传参插入替换
var resInsertRep1 = con.write("/tag/tag3/tag6.his.insertReplace",67.23,"BAD",time1);
...
sleep(2000);
//异步插入替换
var resInsertRepAsy = con.write("/tag/tag3/tag8.his.insert",histry2,function(err,tagName,tagId){
	if(err){
		console.log(err);
	}else{
...
//同步直接传参替换
var resRep1 = con.write("/tag/tag1.his.replace",67.89,"GOOD",time);
...
sleep(2000);
//异步替换
var resRepAsy = con.write("/tag/tag2.his.replace",histry2,function(err,tagName,tagId){
	if(err){
		console.log(err);
	}else{
...
sleep(1000);
//异步查询某一测点固定时间历史数据
var resAtTime2= con.read("/tag/tag1.his.atTime",times,function(err,resVal){
	if(err){
		console.log(err);
	}else{
...
var timeStart = new Date(s1-7200000);
...
var resProcess1 = con.read("/tag/tag2.his.processed",timeStart,timeEnd,10000000,"PS_HIS_START");	
...
sleep(2000);	
//异步查询某一测点一段时间的历史统计数据
var resProcess2 = con.read("/tag/tag3.his.processed",hisPro,function(err,resVal){
	if(err){
		console.log(err);
	}else{
...
//异步直接传参查某一测点历史统计数据
var resProcess2 = con.read("/tag/tag2.his.processed",timeStart,timeEnd,10000000,"PS_HIS_START",function(err,resVal){
	if(err){
		console.log(err);
	}else{
		console.log("异步查询历史统计数据：");
