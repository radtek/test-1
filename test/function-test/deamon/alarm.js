...
var startTime= new Date(t-7200000);
...
//同步查询历史报警
var hisAlarm1 = con.read("/tag/tag3/tag9.alarm.his",hisFilter);
...
//直接传参
var hisAlarm = con.read("/tag/tag1.alarm.his",filterField,filter,startTime,endTime);
...
//异步直接传参查询历史报警
var hisAlarm3 = con.read("/tag/tag4.alarm.his",filterField,filter,startTime,endTime,function(err,hisVal){
	if(err){
		console.log(err);
	}else{
		console.log("异步历史查询报警数量:",hisVal.length);
...
var ackeIds = [2];
//直接传参查询应答报警
var ackAlarm1 = con.read("/tag/tag1.alarm.ack",1,"Admin",startTime,ackeIds);
...
var ackAlarm3 = con.read("/tag/tag3/tag5.alarm.ack",1,"Admin",startTime,ackeIds,function(err,ackVal){
	if(err){
		console.log(err);
	}else{
		console.log("异步应答报警成功!");
