...
Handle<Value> PspaceNode::write(const Arguments& args)
 {
	 ...
	 const char * pstr = ToCString(str);
	 std::vector<std::string> result=split(pstr,".");
	 if(args.Length()==2 && result[1] == "pv" && !fun(args)){
		 return realWriteSyn(args);
	 }else if(args.Length()==3 && result[1] == "pv" && fun(args)){
	 ...
	 }else if (args.Length()==2 && result[1] == "his" && !fun(args))
	 {
		 return hisSyn(args);
	 }else if (args.Length()==3 && result[1] == "his" &&  fun(args))
	 {
		 return hisAsy(args);
	 }
 }
