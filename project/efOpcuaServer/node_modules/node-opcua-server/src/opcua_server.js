...
function OPCUAServer(options) {
    ...
    if(options.applicationName != undefined){
        let j = {};
        j.text = options.applicationName;
        j.locale = null;
        self.serverInfo.applicationName = new LocalizedText(j);
    }
...
OPCUAServer.prototype._on_CreateSubscriptionRequest = function (message, channel) {
    ...
    this._apply_on_SessionObject(CreateSubscriptionResponse, message, channel, function (session, sendResponse, sendError) {
        ...
        subscription.on("monitoredItem", async function (monitoredItem) {
            prepareMonitoredItem(context, addressSpace, monitoredItem);
            //console.log("monitoredItem:",monitoredItem);
            if(monitoredItem._node.tagId!=undefined){
                let tagIds = [];
                let l = {};
                l.tagId = monitoredItem._node.tagId;
                tagIds.push(l);
                let ret = await monitoredItem._node.db.subRealData(tagIds);
                //console.log("ret:",ret);
                let j = {};
                j.tagId = monitoredItem._node.tagId;
                j.subscribeId = ret.retData[0].regNo;
                this.subInfos.set(monitoredItem.monitoredItemId,j);
                //console.log("sub:",JSON.stringify(tagIds));
            }

        });

        subscription.on("subEventAdd",async function(obj){
...
OPCUAServer.prototype._on_DeleteMonitoredItemsRequest = function (message, channel) {
    ...
    this._apply_on_Subscription(DeleteMonitoredItemsResponse, message, channel, function (session, subscription, sendResponse, sendError) {
        ...
        const results = request.monitoredItemIds.map(function (monitoredItemId) {
            //console.log("remove:",monitoredItemId);
