...
Subscription.prototype.createMonitoredItem = function (addressSpace, timestampsToReturn, monitoredItemCreateRequest) {
    ...
    if(subscription.db == undefined){
        subscription.db = node.db;
        subscription.subInfos = new Map();
    }

    if(node.uaps!=undefined){
        let obj = {};
        obj.tagId = node.tagId;
        obj.monitoredItemId = monitoredItemCreateResult.monitoredItemId;
        subscription.emit("subEventAdd",obj);
    }
...
Subscription.prototype.removeMonitoredItem = function (monitoredItemId) {
    ...
    let o = subscription.subInfos.get(monitoredItemId);
    if(o!=undefined){
        subscription.db.unSubRealData([o.subscribeId]);
        //console.log("unsub:",JSON.stringify(o));
    }
    return StatusCodes.Good;
