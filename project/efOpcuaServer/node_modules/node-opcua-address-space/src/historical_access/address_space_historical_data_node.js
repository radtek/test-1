...
function qualityEf2ua(quality){
    ...
    switch(quality){
		...
		case 128:
			...
			break;
		case 4:
			code = StatusCodes.BadConfigurationError;
			break;
		case 8:
			code = StatusCodes.BadServerNotConnected;
			break;
		case 12:
			code = StatusCodes.BadDeviceFailure;
			break;
		case 16:
		...
		case 24:
			...
			break;
		case 28:
			code = StatusCodes.BadSensorFailure;
			break;
		case 32:
			code = StatusCodes.BadWaitingForInitialData;
			break;
		case 68:
			code = StatusCodes.UncertainNoCommunicationLastUsableValue;
			break;
		case 80:
			code = StatusCodes.UncertainSensorNotAccurate;
			break;
		case 84:
			code = StatusCodes.UncertainEngineeringUnitsExceeded;
			break;
		case 88:
			code = StatusCodes.UncertainSubNormal;
			break;
		case 216:
			code = StatusCodes.GoodLocalOverride;
			break;
		case 65536:
			code = StatusCodes.BadTooManyMatches;
			break;
		case 131072:
			code = StatusCodes.GoodEntryInserted;
			break;
		case 262144:
		...
		case 1048576:
			...
			break;
		case 2097152:
			code = StatusCodes.GoodNoData;
			break;
		case 4194304:
			code = StatusCodes.BadDataLost;
			break;
		case 8388608:
			code = StatusCodes.Bad;
			break;
...
exports.install = function (AddressSpace) {
	...
	//modify__
	async function _historyReadRawAsync(
      historyReadRawModifiedDetails,
      maxNumberToExtract,
      isReversed,
      reverseDataValue,
      callback
		...
		//**************添加 modify__******************//
		let t_s = new Date(historyReadRawModifiedDetails.startTime);
        let t_e = new Date(historyReadRawModifiedDetails.endTime);
		let info = "history read " + node.tagId + " " + t_s.toLocaleString() + " " + t_e.toLocaleString();
		node.reqlogger.info(info);
		//console.log(JSON.stringify({'startTime':t_s.toLocaleString(),'endTime':t_e.toLocaleString(),tagIds:[node.tagId]}));
		let retJson = await node.db.getHisData({'beginTime':t_s.toLocaleString(),'endTime':t_e.toLocaleString(),'count':10000,'tagIds':[{'tagId':node.tagId}]});
		//let ss = node.tagId + " " +  JSON.stringify(retJson);
		//console.log(ss);
		
        //console.log("retJson:",JSON.stringify(retJson));
        if(!retJson.retCode){
			let errMsg = "history read error "  + node.tagId + " " + t_s.toLocaleString() + " " + t_e.toLocaleString();
			node.errlogger.error(errMsg);
            callback(null, []);
            return;
        }
		
        let dataValues_ = [];
        for(let i=0; i<retJson.retData[0].values.length;i++){
                if(node.uaDataType_==DataType.Boolean){
                    if(retJson.retData[0].values[i].data>0){
                        retJson.retData[0].values[i].data = true;
                    }else{
                            retJson.retData[0].values[i].data = false;
                        }
                    }
    
                    let dataValue = new DataValue({
                        value: new Variant({
                            dataType: node.uaDataType_,
                            arrayType: VariantArrayType.Scalar,
                            dimensions: null,
                            value:retJson.retData[0].values[i].data}),
                            serverTimestamp: new Date(retJson.retData[0].values[i].time),
                            serverPicoseconds: 0,
                            sourceTimestamp: new Date(retJson.retData[0].values[i].time),
                            sourcePicoseconds: 0,
                            statusCode: qualityEf2ua(retJson.retData[0].values[i].quality)});
			
				dataValues_.push(dataValue);
			
		}
		//*********************************************//
		callback(null, dataValues_);
    ...
    async function _historyRead(context, historyReadDetails, indexRange, dataEncoding, continuationPoint, callback) {
        ...
        if (historyReadDetails instanceof ReadRawModifiedDetails) {
            ...
            console.log(JSON.stringify(historyReadDetails));
            ...
            let arrJson = {};
            arrJson.timeStamps = historyReadDetails.reqTimes;
            arrJson.tags = [];
            arrJson.tags.push(node.tagId);
            let retJson = await node.db.getHisReadAtTime(arrJson);
            if(retJson.retCode!=0 && retJson.retCode!=-19997){
                let errMsg = "history readattime error "  + node.tagId + " " + retJson.retCode;
                node.errlogger.error(errMsg);
                return callback(null, []);
            }

            let dataValues = [];
            for(let i=0; i<retJson.tagHisData[0].tagDatas.length;i++){
                let dataValue = new DataValue({
                    value: new Variant({
                        dataType: node.uaDataType_,
                        arrayType: VariantArrayType.Scalar,
                        dimensions: null,
                        value:retJson.tagHisData[0].tagDatas[i].value.data}),
                        serverTimestamp: new Date(retJson.tagHisData[0].tagDatas[i].time),
                        serverPicoseconds: 0,
                        sourceTimestamp: new Date(retJson.tagHisData[0].tagDatas[i].time),
                        sourcePicoseconds: 0,
                        statusCode: StatusCodes.Good});
						//retJson.tagHisData[0].tagDatas[i].quality(暂未转换)
