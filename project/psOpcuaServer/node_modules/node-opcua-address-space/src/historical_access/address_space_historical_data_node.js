...
exports.install = function (AddressSpace) {
	...
	async function _historyReadRawAsync(
      ...
      callback
        ...
        let bouds = 0;
        if(historyReadRawModifiedDetails.returnBounds){
            bouds = 1;
        }
		
		//console.log(JSON.stringify({'startTime':t_s.toLocaleString(),'endTime':t_e.toLocaleString(),tagIds:[node.tagId]}));
		let retJson = await node.db.getHisReadRaw({'startTime':t_s.toLocaleString(),'endTime':t_e.toLocaleString(),tagIds:[node.tagId]});
		//let ss = node.tagId + " " +  JSON.stringify(retJson);
		//console.log(ss);
		
		
        if(retJson.retCode!=0 && retJson.retCode!=-19997 && retJson.retCode!=-18897){
			let errMsg = "history read error "  + node.tagId + " " + t_s.toLocaleString() + " " + t_e.toLocaleString() + " " +retJson.retCode;
        ...
        let dataValues_ = [];
        for(let i=0; i<retJson.tagHisDatas[0].dataSet.length;i++){
                let dataValue = new DataValue({
                    value: new Variant({
                        dataType: node.uaDataType_,
                        arrayType: VariantArrayType.Scalar,
                        dimensions: null,
                        value:retJson.tagHisDatas[0].dataSet[i].value}),
                        serverTimestamp: new Date(retJson.tagHisDatas[0].dataSet[i].time),
                        serverPicoseconds: 0,
                        sourceTimestamp: new Date(retJson.tagHisDatas[0].dataSet[i].time),
                        sourcePicoseconds: 0,
                        statusCode: StatusCodes.Good});
						//retJson.tagHisDatas[0].dataSet[i].quality(暂未转换)
